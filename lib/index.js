...
module.exports = function(chai, utils) {
  ...
  Assertion.addMethod('locals', function(locals) {
    ...
    if (contains && (any !== undefined || all !== undefined)) {
      ...
      // If `.include` is combined with `.all`, the target keys only need to be
      // a superset of the expected keys.  However, since `.include` is being
      // reinterpreted here as "defined", an identical set is required.  Thus,
      // the `contains` flag is explicitly disabled.
      utils.flag(assertion, 'contains', false);
      assertion.have.keys(locals);
    } else if (contains) {
      assertion = new Assertion(Object.keys(obj.locals));
      utils.transferFlags(this, assertion, false);
      assertion.include.members(locals);
    } else if (deep) {
      assertion = new Assertion(obj.locals);
      utils.transferFlags(this, assertion, false);
