...
module.exports = function(chai, utils) {
  ...
  chai.express.use = function(callbacks) {
   ...
   * @api public
   */
  Assertion.addMethod('body', function(body) {
    var obj = this._obj;
    ...
    if (typeof body == 'object') {
      assertion = new Assertion(obj.body);
      utils.transferFlags(this, assertion, false);
      assertion.deep.equal(body);
    } else {
      this.assert(
          obj.body === body
        , 'expected response to have body #{exp} but got #{act}'
        , 'expected response to not have body #{act}'
        , body  // expected
        , obj.body  // actual
      );
    }
  ...
  Assertion.addMethod('render', function(view) {
    ...
    new Assertion(obj).to.be.an.instanceof(Response);
    new Assertion(obj).to.have.property('_view');
